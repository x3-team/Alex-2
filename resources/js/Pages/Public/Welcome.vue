    ...
    >
        ...
        >
            <button
                ...
                type="button"
                @click.prevent="visitPath(isDoctorMode ? '/' : '/doctors')"
            >
            ...
            <div
                ...
                :class="{
                    ...
                    'doctor-backdrop-visible': isDoctorMode && currentIndex >= 2 && currentIndex <= 10,
                    'results-top-backdrop-visible': !isDoctorMode && currentIndex >= 7 && currentIndex <= 10,
                    'mobile-backdrop-visible': currentIndex >= 2 && currentIndex <= 10,
                }"
            >
                <button
...
const { isDoctorMode, toggleAudienceMode } = useDoctorMode();
...
const showMobileInfoBackground = computed(() => isMobile.value && currentIndex.value >= 12);
...
const goToPersistentStep = (step) => {
    ...
    const base = persistentSteps.value.type === 'advantages' ? 2 : 8;
...
const activeMenuIndex = computed(() => {
    ...
    if (index >= 7 && index <= 10) return 1;
    if (index === 11) return 2;
    if (index === 12) return 3;
    if (index >= 13 && index <= 15) return 4;
...
const mobileCloudClass = computed(() => {
    const index = currentIndex.value;
    const large = (index >= 2 && index <= 6) || (index >= 8 && index <= 10);
    return index === 0 || large ? 'mobile-cloud-large' : 'mobile-cloud-compact';
});
...
const showSmokeCloud = computed(() => {
    ...
    if (isDoctorMode.value) {
        const advantages = index >= 2 && index <= 6;
        const results = index >= 7 && index <= 10;
        if (!advantages && !results) {
            return false;
        }
...
const isResultsCloud = computed(() => {
    const index = currentIndex.value;
    return index >= 8 && index <= 10;
});

const resultsCloudClass = computed(() => {
    const index = currentIndex.value;
    if (index === 7) return 'results-intro-cloud';
    if (index >= 8 && index <= 10) return 'results-cloud-primary';
    return '';
});

const persistentSteps = computed(() => {
    const index = currentIndex.value;
    if (index >= 2 && index <= 6) {
        return { type: 'advantages', total: 5, current: index - 1 };
    }
    if (index >= 8 && index <= 10) {
        return { type: 'results', total: 3, current: index - 7 };
    }
    return null;
});
...
watch(currentIndex, (nextIndex, prevIndex) => {
    ...
    const isHowToPass = (index) => index >= 13 && index <= 15;
    ...
    if (jumped) {
        if (nextSrc) {
            playSrcOnIncoming(nextSrc, { onPlaybackStarted, onComplete: finishNow });
        } else {
